"""Stage 1B v2 inference — supports the prepare_line_dataset_v2.py channel set.

Differences from `infer_line_mask_unet.py`:
- Sato uses ``sato(raw, black_ridges=True)`` (dark-ridge polarity matches
  prepare_v2). v1 uses the bright-ridge polarity which empirically picks up
  brightness *between* dark Kossel lines rather than the lines themselves.
- Adds **black_score** channel support: loaded from
  ``<dataset>/responses/response_black_score.tif`` if cached, otherwise
  computed on-the-fly.
- Same checkpoint + sliding-window pipeline as v1 (UNet imported from
  train_line_mask_unet).

Run::

    python infer_line_mask_unet_v2.py \\
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Stage 1B v2 U-Net inference")
    p.add_argument("--model", required=True)
    p.add_argument("--dataset", required=True,
                   help="Stage 1 v2 dataset root for cached responses & label compare")
...
def compute_black_line_score(
    ...
    return (np.clip(dark_residual, 0.0, None) / np.maximum(noise, 1e-6)).astype(np.float32)


def load_response_v2(rdir: Path, name: str, img: np.ndarray, smooth_raw: np.ndarray,
                     smooth_abs: np.ndarray, sigmas) -> np.ndarray:
    p = rdir / f"response_{name}.tif"
    if p.exists():
        return tifffile.imread(str(p)).astype(np.float32)
    print(f"[INFO] {p.name} missing → recomputing")
    if name == "sato":
        return sato_filter(smooth_raw, sigmas=sigmas, black_ridges=True).astype(np.float32)
    if name == "coherence":
        return structure_coherence(smooth_raw, 2.0, 8.0).astype(np.float32)
    if name == "gradient":
        return gradient_magnitude(smooth_abs).astype(np.float32)
    if name == "black_score":
        return compute_black_line_score(img, sigma_bg=20.0, sigma_noise=8.0)
    raise ValueError(f"unknown response name: {name}")


def build_channel_stack(
    input_path: Path, dataset: Path, channel_names: list[str],
    ...
    black_p99 = float(summary.get("black_score_norm_scale_p99_5") if summary else 0.0)
    ...
    def get(name: str) -> np.ndarray:
        ...
        elif name == "black_score":
            r = load_response_v2(rdir, name, img, smooth_raw, smooth_abs, sigmas)
            scale = black_p99 if black_p99 > 0 else float(np.percentile(r, 99.5))
            v = np.clip(r / max(scale, 1e-6), 0.0, 1.0).astype(np.float32)
        elif name in ("sato", "coherence", "gradient"):
            r = load_response_v2(rdir, name, img, smooth_raw, smooth_abs, sigmas)
...
def sliding_window_predict(
    model: torch.nn.Module, image_chw: np.ndarray,
    patch_size: int, stride: int, device: torch.device,
) -> np.ndarray:
...
def load_checkpoint_and_config(model_path: Path, override_cfg: Path | None):
    ...
    channel_names = ckpt.get(
        "channel_names",
        cfg.get("channel_names", ["raw", "abs", "sato", "black_score", "coherence", "gradient"]),
    )
...
def main() -> None:
    ...
    ckpt, cfg, in_channels, base_channels, channel_names = load_checkpoint_and_config(
        model_path, cfg_override,
    )
    ...
    # Optionally read prepare_v2 summary for black_score scaling
    summary_path = dataset / "reports" / "summary.json"
    summary: dict | None = None
    ...
    raw_img, stack = build_channel_stack(input_path, dataset, channel_names, summary)
    H, W = raw_img.shape
    ...
    for thr in args.thresholds:
        ...
        print(f"[INFO] prob > {thr}: coverage = {mask.mean()*100:.2f}%")

    # Compare against selected label (if available)
    ...
    black_resp_path = dataset / "responses" / "response_black_score.tif"
    black_disp = None
    if black_resp_path.exists():
        bls = tifffile.imread(str(black_resp_path)).astype(np.float32)
        black_disp = np.clip(bls / max(float(np.percentile(bls, 99.5)), 1e-6), 0.0, 1.0)

    panels = []
    panels.append((to_rgb(raw_norm), "input"))
    if sato_disp is not None:
        panels.append((to_rgb(sato_disp), "response_sato (dark-ridge)"))
    if black_disp is not None:
        panels.append((to_rgb(black_disp), "black_line_score"))
    if sel_label is not None and sel_label.shape == raw_img.shape:
        ...
        panels.append((rgb_lab, "train label overlay"))
    ...
    for i in range(rows * cols):
        ...
        if img_panel.ndim == 2:
            ax.imshow(img_panel, cmap="magma" if title == "line_prob" else "gray",
                      vmin=0, vmax=1)
        else:
    ...
    print(f"  prob coverage @ 0.3 = {(prob > 0.3).mean()*100:.2f}%")
    print(f"  prob coverage @ 0.5 = {(prob > 0.5).mean()*100:.2f}%")
    print(f"  prob coverage @ 0.7 = {(prob > 0.7).mean()*100:.2f}%")
    print(f"  prob p50={np.percentile(prob, 50):.3f}, p90={np.percentile(prob, 90):.3f}, p99={np.percentile(prob, 99):.3f}")
